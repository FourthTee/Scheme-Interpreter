...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            ...
            map_func = lambda operand: scheme_eval(operand,env)
            operands = rest.map(map_func)
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if vals is not nil:
                ...
                raise SchemeError('More Formals than Vals')
        if vals is not nil:
            raise SchemeError('More Vals than Formals')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        func_value = do_lambda_form(Pair(target.second, expressions.second), env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    bindings_check = bindings
    while bindings_check is not nil:
        check_form(bindings_check.first, 2, 2)
        bindings_check = bindings_check.second
    formals = bindings.map(lambda symbols: symbols.first)
    check_formals(formals)
    vals = bindings.map(lambda val: scheme_eval(val.second.first, env))
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        macro_formals = target.second
        macro_body = expressions.second
        macro = MacroProcedure(macro_formals, macro_body, env)
